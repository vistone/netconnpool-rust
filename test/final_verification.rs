...
fn test_forced_eviction_of_leaked_connections() {
    ...
    assert!(conn2.is_ok(), "Should be able to get a new connection after eviction. Error: {:?}", conn2.err());
}
...
fn test_udp_buffer_clearing_on_get() {
    ...
    assert!(result.is_err(), "Buffer should have been cleared, but received: {:?}", result);
}
...
fn test_pool_closure_reaper_exit() {
    ...
    assert!(elapsed < Duration::from_millis(50), "Pool took too long to close: {:?}", elapsed);
}
