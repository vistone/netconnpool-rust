...
fn benchmark_connection_creation() {
    ...
    println!("  平均耗时: {:?} ms/conn", creation_time.as_millis() / num_connections);
    println!("  创建速率: {:.2} conn/sec", num_connections as f64 / creation_time.as_secs_f64());
    
    // 创建100个连接应该在合理时间内完成
    assert!(creation_time < Duration::from_secs(10), "创建连接应该在10秒内完成");
}
