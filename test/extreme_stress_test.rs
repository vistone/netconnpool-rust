...
fn test_extreme_stress_all_scenarios() {
    ...
    println!("💾 内存监控配置:");
    println!("   系统总内存: {} MB ({:.1} GB)", total_memory_mb, total_memory_mb as f64 / 1024.0);
    println!("   内存限制: {} MB ({:.1} GB)", memory_limit_mb, memory_limit_mb as f64 / 1024.0);
    println!("   安全阈值: 80%\n");
    ...
    for (scenario_name, threads, requests_per_thread, data_size, tcp_port, udp_port) in scenarios {
        ...
        if !memory_monitor.check_limit() {
            ...
            println!("⚠️  内存使用已达限制: {} MB / {} MB (峰值: {} MB)", 
    ...
    println!("   使用率: {:.1}%", final_peak as f64 / memory_limit_mb as f64 * 100.0);
    
    println!("\n📊 各场景测试结果:");
    for (name, result) in &all_results {
        if result.success {
            println!("  ✅ {}: 成功率 {:.2}%, QPS {:.0}, 复用率 {:.2}%", 
...
fn run_extreme_scenario(
                                ...
                                match stream_ref.try_clone() {
                                    Ok(mut stream) => {
                                        if stream.write_all(&data).is_err() {
                                            ...
                                            stats.latency_stats.record(req_start.elapsed(), false, false);
                                            continue;
                                        }
                                        ...
                                        stats.latency_stats.record(req_start.elapsed(), true, false);
                                    }
                                    Err(_) => {
                                        stats.failed_requests.fetch_add(1, Ordering::Relaxed);
                                        stats.latency_stats.record(req_start.elapsed(), false, false);
    ...
    for (idx, handle) in handles.into_iter().enumerate() {
        ...
        if idx % 200 == 0 && idx > 0 {
            ...
            println!("    线程完成进度: {}/{} ({:.1}%)", idx, num_threads, progress);
        }
    ...
    // 获取响应时间和丢包率统计
    let (avg_latency, min_latency, max_latency, p50, p95, p99, packet_loss_rate) = latency_stats.get_stats();
    ...
    let test_passed = success_rate > 99.0 && reuse_rate > 95.0 && packet_loss_rate < 1.0 && !stop_flag.load(Ordering::Relaxed);
    ...
    println!("  失败请求: {}", client_stats.failed_requests.load(Ordering::Relaxed));
    println!("  超时请求: {}", client_stats.timeout_errors.load(Ordering::Relaxed));
