                        ...
                        loop {
                            match stream.read(&mut buffer) {
                                ...
                                Ok(n) => {
                                    ...
                                    stats_clone.total_bytes_received.fetch_add(n, Ordering::Relaxed);
...
fn test_real_world_max_stress() {
    ...
    println!("   总数据量: {:.2} GB", (total_requests * data_size * 2) as f64 / 1024.0 / 1024.0 / 1024.0);
    ...
    thread::spawn(move || {
        ...
        while !monitor_stop_clone.load(Ordering::Relaxed) {
            ...
            let overall_success_rate = if total > 0 { success as f64 / total as f64 * 100.0 } else { 100.0 };
            
            println!("\n╔════════════════════════════════════════════════════════════════╗");
            println!("║           实时统计 - 第{}个5秒间隔 ({:.1}s)                    ║", interval_count, elapsed);
            println!("╚════════════════════════════════════════════════════════════════╝");
            println!("客户端:");
            println!("  瞬时QPS: {:.0} req/s (最近5秒)", interval_qps);
            println!("  平均QPS: {:.0} req/s (最近{}个间隔)", avg_qps, hist_len.min(100));
            println!("  总体QPS: {:.0} req/s (累计)", overall_qps);
            println!("  总请求: {} ({:.1}%)", total, (total as f64 / total_requests as f64 * 100.0));
            ...
            println!("  平均成功率: {:.2}% (最近{}个间隔)", avg_success_rate, hist_len.min(100));
            println!("  吞吐量: {:.2} MB/s", 
                (stats_monitor.total_bytes_sent.load(Ordering::Relaxed) + 
                 stats_monitor.total_bytes_received.load(Ordering::Relaxed)) as f64 
            ...
            println!("  吞吐量: {:.2} MB/s", 
                (server_monitor.total_bytes_received.load(Ordering::Relaxed) + 
                 server_monitor.total_bytes_sent.load(Ordering::Relaxed)) as f64 
                / 1024.0 / 1024.0 / elapsed);
            
            // 性能稳定性检查
            if interval_count >= 10 { // 至少运行50秒后开始检查
                let qps_variance = {
                    let qps_hist = qps_history_clone.lock().unwrap();
                    if qps_hist.len() > 1 {
                        let mean = avg_qps;
                        let variance = qps_hist.iter()
                            .map(|&x| (x - mean).powi(2))
                            .sum::<f64>() / qps_hist.len() as f64;
    ...
    println!("  连接错误: {}", client_stats.connection_errors.load(Ordering::Relaxed));
    println!("  超时错误: {}", client_stats.timeout_errors.load(Ordering::Relaxed));
    ...
    println!("  吞吐量: {:.2} MB/s", 
        ...
        / 1024.0 / 1024.0 / total_time.as_secs_f64());
    ...
    println!("\n📊 服务器统计:");
    println!("  总连接数: {}", server_stats.total_connections.load(Ordering::Relaxed));
    println!("  当前活跃: {}", server_stats.active_connections.load(Ordering::Relaxed));
    println!("  总请求数: {}", server_stats.total_requests.load(Ordering::Relaxed));
    ...
    let (qps_variance, avg_success_rate, _avg_reuse_rate) = {
        ...
        let avg_reuse = if !reuse_hist.is_empty() {
            reuse_hist.iter().sum::<f64>() / reuse_hist.len() as f64
        } else { (tcp_reuse_rate + udp_reuse_rate) / 2.0 };
        
        (qps_var, avg_success, avg_reuse)
    };
    
    // 高标准验证（长时间运行要求）
    assert!(success_rate > 99.9, "成功率应该超过99.9%，实际: {:.2}%", success_rate);
    assert!(avg_success_rate > 99.9, "平均成功率应该超过99.9%，实际: {:.2}%", avg_success_rate);
    assert!(qps > 50000.0, "QPS应该超过50,000，实际: {:.0}", qps);
    assert!(tcp_reuse_rate > 99.0, "TCP连接复用率应该超过99%，实际: {:.2}%", tcp_reuse_rate);
    assert!(udp_reuse_rate > 99.0, "UDP连接复用率应该超过99%，实际: {:.2}%", udp_reuse_rate);
    assert!(qps_variance < 30.0, "QPS变异系数应该小于30%，实际: {:.2}%", qps_variance);
    ...
    // 长时间运行性能保证
    if success_rate > 99.9 && avg_success_rate > 99.9 && tcp_reuse_rate > 99.0 && udp_reuse_rate > 99.0 && qps_variance < 30.0 {
        println!("\n🎉 长时间运行性能保证: ✅ 100% 通过");
